...
{
    [MenuItem("ScreenShoot/Capture(Ctrl +2) _2")]
    public static void Capture()
    {

        var path = Path.Combine(GetScreenShootPath(), System.Guid.NewGuid().ToString("N") + ".png");
        ScreenCapture.CaptureScreenshot(path);
        Debug.Log("Captured:" + path);
    }
    [MenuItem("ScreenShoot/Show ScreenShoot Folder")]
    public static void OpenScreenShootFolder()
    {
        OpenInFileBrowser.Open(GetScreenShootPath());
    }

    private static string GetScreenShootPath()
    {
        var projectFolder = Directory.GetParent(Application.dataPath);
        var screenShootPath = Path.Combine(projectFolder.FullName, "ScreenShoot");
